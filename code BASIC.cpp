...
bool processCommand(const string& command) {
    ...
    // Goto
        ...
        if (program.find(stoi(tokens[1])) != program.end()) {
...
void runProgram() {
    while (program.find(currentLine) != program.end() && !haltProgram) {
        ...
        // Correção: Atualizar corretamente o `currentLine` se nenhum `goto` ou `halt` for executado
        if (!lineChanged) {
            currentLine += 10; // Avança para a próxima linha se não houver mudança
        }
        
        // Log para verificar valores de A e S após cada iteração
        if (variables.find("A") != variables.end()) {
            cout << "Valor de A após iteração: " << variables["A"] << endl;
        }
        if (variables.find("S") != variables.end()) {
            cout << "Valor de S após iteração: " << variables["S"] << endl;
        }
    }
}
